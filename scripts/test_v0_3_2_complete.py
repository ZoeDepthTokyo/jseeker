...
test(
    "Version in config.py",
    settings.app_version == "0.3.2",
...
test(
    ...
    jseeker.__version__ == "0.3.2",
    f"Found: {jseeker.__version__}"
)
...
has_robust_parsing = "market_countries" in discovery_content or "parse_location_hierarchy" in discovery_content
test(
...
test(
    ...
    backfill_script.exists(),
    f"Path: {backfill_script}"
)
...
test(
    ...
    limitations_doc.exists(),
    f"Path: {limitations_doc}"
)
...
no_separate_folders = "pdf_folder" not in resume_lib_content or "docx_folder" not in resume_lib_content
